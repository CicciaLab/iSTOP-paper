...
TUSCON <-
  ...
  select(gene = Gene, q_TSG_TUSCON = TUSON.combined.qvalue.TSG)

TSG <- inner_join(Twenty_Twenty, TUSCON)
...
Comparison <-
  ...
  mutate(
    score_iSTOP  = -log10(q),
    score_TUSCON = -log10(q_TSG_TUSCON),
    score_20_20  = -log10(q_TSG_20_20),
    TSG_iSTOP    = score_iSTOP  >= sort(score_iSTOP,  decreasing = T)[100],
    TSG_TUSCON   = score_TUSCON >= sort(score_TUSCON, decreasing = T)[100],
...
Comparison %>%
  filter(TSG_iSTOP | TSG_TUSCON | TSG_20_20 | TSG_CGC) %>%
  write_csv('data/Figure-data/Figure-ST5.csv')
