...
codon <- read_csv('data/Figure-data/Figure-4A.csv') %>% mutate(species = ordered(species, levels = species_order))
tx    <- read_csv('data/Figure-data/Figure-4B.csv') %>% mutate(species = ordered(species, levels = species_order))
gene  <- read_csv('data/Figure-data/Figure-4C.csv') %>% mutate(species = ordered(species, levels = species_order))
...
ggsave('figures/Figure-4A.pdf', gg1, height = 4, width = 11)


# ---- ORFs ----

gg_transcripts_targetable <-
  tx %>%
...
gg_transcripts_untargetable <-
  ...
  scale_y_reverse(breaks = seq(0, 1000, by = 200), expand = c(0, 0), labels = scales::comma) +
  scale_x_discrete(position = 'top') +
...
ggsave('figures/Figure-4B.pdf', gg2, height = 4, width = 11)
...
ggsave('figures/Figure-4C.pdf', gg3, height = 4, width = 11)
